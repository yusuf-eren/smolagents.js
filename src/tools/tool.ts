...
// Adding typing would be better. new Tool<T>({ ... }) for input types.
const egTool = new Tool({
  name: 'test',
  description: 'test',
  inputs: {
    test: {
  ...
  outputType: 'string', // TODO: Add support for zod types.
});

egTool.execute = (input: any, r: number): any => {
  r;
  return input;
};
...
await rq.execute('test').then(console.log);
